          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`px-2.5 py-1 rounded text-[10px] font-barlow font-black tracking-[2px] uppercase transition-all duration-150 ${
                  isActive
                    ? "text-[#0EA5E9] bg-[#0EA5E9]/10 shadow-[inset_0_0_0_1px_rgba(14,165,233,0.2)]"
                    : "text-[#4A6278] hover:text-[#8AAFCC] hover:bg-[#0EA5E9]/5"
                }`}
              >
                {item.label}
              </Link>
